...
bj_lst = ['Beijing', 'Beijing 1', 'Beijing 2', 'Beijing 3']
cd_lst = ['Chengdu', 'Chengdu 1']
cq_lst = ['Chongqing', 'Chongqing 1']
dl_lst = ['Dalian', 'Dalian 1']
fj_lst = ['Fujian', 'Fujian 1']
gs_lst = ['Gansu', 'Gansu 1']
gz_lst = ['Guangzhou', 'Guangzhou 1', 'Guangzhou 2']
hb_lst = ['Haerbin', 'Haerbin 1']
hz_lst = ['Hangzhou', 'Hangzhou 1']
hlj_lst = ['Heilongjiang', 'Heilongjiang 1', 'Heilongjiang 2']
ht_lst = ['Huhehaote', 'Huhehaote 1']
hn_lst = ['Hunan', 'Hunan 1']
jl_lst = ['Jilin', 'Jilin 1']
nj_lst = ['Nanjing', 'Nanjing 1']
sh_lst = ['Shanghai', 'Shanghai 1']
sz_lst = ['Shenzhen', 'Shenzhen 1']
sjz_lst = ['Shijiazhuang', 'Shijiazhuang 1']
tj_lst = ['Tianjin', 'Tianjin 1', 'Tianjin 2', 'Tianjin 3']
wh_lst = ['Wuhan', 'Wuhan 1']
zj_lst = ['Zhejiang', 'Zhejiang 1']

multi_lst = [bj_lst, cd_lst, cq_lst, dl_lst, fj_lst, gs_lst, gz_lst, hb_lst, hz_lst, hlj_lst, ht_lst, hn_lst, jl_lst, nj_lst, sh_lst, sz_lst, sjz_lst, tj_lst, wh_lst, zj_lst]
...
def drop_contradic_states(df_, contrad_dic, study_city, tuple_=False, drop_id=0): # id==0 means drop cities, id==1 means drop states
    ...
    if tuple_==True:
        final_contra_states = []
        temp_contra_states = list(np.unique([c[drop_id] for c in contra_states]))
        
        for cs in temp_contra_states:
            find_multi = False
            for cs_lst in multi_lst:
                if cs in cs_lst:
                    study_city = study_city.split('--')[0]
                    while study_city in cs_lst:
                        cs_lst.remove(study_city)     
                    final_contra_states.extend(cs_lst)
                    find_multi = True
                    break
            if find_multi:
                continue
            else:
                final_contra_states.append(cs)

    if len(contra_states)>0:
        df_.drop(columns=final_contra_states, inplace=True)
        print('{} dropped states or cities: {}'.format( len(final_contra_states), ', '.join(final_contra_states)))
    return df_

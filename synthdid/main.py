...
if __name__ == "__main__":
    ...
    sector = 'industry' # industry power
    ...
    for i, row in lock_down_info.iterrows():
        ...
        syn_res['extreme'] = np.round(redundancy[0], 3)
        # syn_res['extreme_in'] = np.round(redundancy[1], 3)
        # syn_res['extreme_pos'] = np.round(redundancy[2], 3)
        ...
        city_res_data = pd.DataFrame( {'cf' : counter_facts, 'ts' : true_ys}, index=emission_resi_.index )
        city_res_data['begin_idx' ] = (city_res_data.index==lock_begin)
        city_res_data['end_idx' ] = (city_res_data.index==lock_end)

...
def eval_resi_vals(tys_period, cfs_period, period='in'):   
    ...
    if period=='in':
        len_ = int((tys_period.shape[0]/4)*3)
        tys_period = tys_period[len_:]
        cfs_period = cfs_period[len_:]
        baseline = np.min( (np.min(tys_period), np.min(cfs_period)) )
        cfs_baseline = cfs_period - baseline
        tys_baseline = tys_period - baseline
        
        diff = tys_baseline - cfs_baseline
        max_dif_locs = np.argsort(np.abs(diff))[-2:]
        
        tys_diff_val = np.mean(tys_baseline[max_dif_locs])
        cfs_diff_val = np.mean(cfs_baseline[max_dif_locs])
        max_diffs = np.mean(np.abs(diff)[max_dif_locs])
    ...
    redundancy = 1 - max_diffs/(max(tys_diff_val, cfs_diff_val))
...
def cal_resilience(df_, origin_df, counter_facts, treated, begin_, end_):
    ...
    resi_all_abs, redun_all = eval_resi_vals(tys_period, cfs_period, period='all')
    resi_in_abs, redun_in = eval_resi_vals(tys_period_in, cfs_period_in, period='in')
    resi_af_abs, redun_pos = eval_resi_vals(tys_period_after, cfs_period_after, period='pos')
    ...
    return (resi_all_abs, resi_in_abs, resi_af_abs), (redun_all, redun_in, redun_pos), (att_, att_in, att_pos), (year_diff, tys_last_year)
...
if __name__ == "__main__":
    ...
    sector = 'industry' # power industry
    ...
    resi_df.to_csv(os.path.join(parent_dir, 'results/' + sector + '_resilience.csv'), index=False)
    df4regions.to_csv(os.path.join(parent_dir, 'results/' + sector + '_radar.csv'), index=False)

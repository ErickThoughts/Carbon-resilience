...
if __name__ == "__main__":
    ...
    threshold_contin = 0.01 # 0.05 0.01
    threshold_disc = 0.1
    ...
    if len(sig_pd_chiqs)>0:
        sig_pth = os.path.join(parent_dir, 'results/' + sector + '_chisquire.csv' )
        sig_pd_chiqs.to_csv(sig_pth, mode='w', index=False)
    elif len(sig_pd_spear)>0:
        sig_pth = os.path.join(parent_dir, 'results/' + sector + '_spearman.csv' )
